...
export default class extends TestableScene {
  ...
  private setupGUI() {
    ...
    settings.add(this.params, 'centerX', -4, 4, 0.1).onChange(() => this.generate());
    settings.add(this.params, 'centerY', -4, 4, 0.1).onChange(() => this.generate());
    settings.open();
    gui.add({ regenerate: () => this.generate() }, 'regenerate');
  }

  private generate() {
    const points = getRandomPointsAroundPoint({
