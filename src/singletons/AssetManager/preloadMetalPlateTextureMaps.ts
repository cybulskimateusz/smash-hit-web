...
const preloadMetalPlateTextureMaps = async (): Promise<TextureMaps> => {
  ...
  const [map, displacementMap, roughnessMap, normalMap, metalnessMap] = await Promise.all([
    textureLoader.loadAsync('/src/assets/metal_plate_material/diffuse.jpg'),
    textureLoader.loadAsync('/src/assets/metal_plate_material/displacement.png'),
    exrLoader.loadAsync('/src/assets/metal_plate_material/roughness.exr'),
    exrLoader.loadAsync('/src/assets/metal_plate_material/normal.exr'),
    exrLoader.loadAsync('/src/assets/metal_plate_material/metalness.exr')
  ]);
